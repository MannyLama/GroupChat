					...
					if(senderGroup.getOwner().equals(pl.getUniqueId())) {
						if(Bukkit.getPlayer(args[1]) != null) {
							if(!senderGroup.isBanned(Bukkit.getPlayer(args[1]))) {
								...
								else { Messenger.send(sender, Messages.INFO_PLAYER_ALREADY_MEMBER); }
							}
							else { Messenger.send(sender, Messages.INFO_PLAYER_IS_BANNED.replaceAll("<player>", Bukkit.getPlayer(args[1]).getName())); }
						}
						else { Messenger.send(sender, Messages.INFO_PLAYER_NOT_ONLINE); }
					}
					else { Messenger.send(sender, Messages.INFO_YOU_NOT_OWNER); }
				}
				else { Messenger.send(sender, Messages.INFO_YOU_NOT_MEMBER); }
			}
			else { Messenger.send(pl, Messages.INFO_NO_PERMISSION); }
		}
